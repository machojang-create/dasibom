    ...
    function load(){
      try{
        firebase.firestore().collection('comments').where('scope','==',scope).limit(80).get()
          .then(function(snap){ var arr=[]; snap.forEach(function(d){ var x=d.data(); arr.push({text:x.text, ts:(x.ts&&x.ts.toMillis)?x.ts.toMillis():0}); }); arr.sort(function(a,b){return b.ts-a.ts;}); render(arr); })
