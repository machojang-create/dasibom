...
| 카테고리 | 비중 | 독자 | CTA 목적지 |
|---|---|---|---|
| 부모님과의 시간 | 30% | 40~50대 자녀 | `/memoir` 자서전 |
| 건강한 노후 | 20% | 60대 + 자녀 | `/health` 건강돋보기 |
| 돌봄 정책과 현장 | 20% | 지자체·복지관 | `/kiosk` 도입 문의 |
| 취미와 반려생활 | 15% | 60대 + 자녀 | `/plant` 수다쟁이 화분 |
| 어르신들의 이야기 | 10% | 전 연령 | `/memoir` (**수동 작성**) |
| 오늘의 즐거움 | 5% | 60대 본인 | `/arcade` 오락실 |
...
```json
{ "id": "P13", "category": "hobby_pet", "cta_key": "guppy", ... }
```

취미·반려 카테고리가 특히 그렇습니다. 원예 글은 `plant`, 어항 글은 `guppy` 가 자연스럽지만, **실제 강아지·고양이 글에 "가상 화분 키우기" 를 붙이면 연결이 약합니다.** 앱에 실제 반려동물 기능이 없으므로 그런 글은 `memoir`(반려동물과의 시간을 기록) 쪽으로 보내거나, 못 키우는 상황의 대안으로 `plant` 를 제시하는 각도로 씁니다. 큐의 P01·P05·P06 이 그 예입니다.
